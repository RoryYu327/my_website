...
ggplot(tidyweather, aes(x=date, y = delta))+
  ...
  labs (
    ...
    x = "Year",
    y = "Deviation of temperature (from expected)"
  )
...
Per the graphs, we can see increasing temperature more pronounced in winter months with March showing the highest deviation from expected temperature. This implies temperatures have changed the most in winter months compared to the temperatures between 1951-1980.  
...
ggplot(comparison, aes(x=delta, fill=interval))+
  ...
  labs (
    ...
    x = "Deviation of temperature (from expected)"
  )
```

So far, we have been working with monthly anomalies. However, we might be interested in average annual anomalies. We can do this by using `group_by()` and `summarise()`, followed by a scatter plot to display the result. 

```{r averaging annual anomalies}
...
ggplot(average_annual_anomaly, aes(x=Year, y= annual_average_delta))+
  ...
  labs (
    ...
    y = "Average Annual Deviation of temperature (from expected)"
  )                         
...
Firstly, to get the summary statistics, we used a formula to calculate the mean, standard deviation, standard error and the confidence interval. To confirm our results, we then used bootstrap simulation to create 1000 samples of our own and recalculate the simulated sample statistics.  
From the mean of delta of 1.06, we can say that since 2011 till present, the average annual temperature deviation is 1.06 degrees from the temperatures between 1951-1980. This is a significant change because it takes a vast amount of heat to warm all the oceans, atmosphere, and land by that much. Looking at the 95% confidence interval, we can say that we are 95% confident that the average annual deviation from expected temperature lies between 1.01 and 1.11.  
...
No, these two confidence intervals do not overlap. There is a significant difference in the proportion of bachelor graduates who do and who do not use Twitter. Therefore, we can make inferences about twitter usage according to whether people are bachelor graduates or not.  
...
Increasing the confidence level from 95% to 99% will increase the margin of error and result in a wider interval.  
...
- Who did you collaborate with: Jiangxia Yu; Marianna Taki; Yuting Wu; Ferdinand Wohlenberg; Massimiliano Ursig; Tanisha Yadav
- Approximately how much time did you spend on this problem set: 7 hours
- What, if anything, gave you the most trouble: Recreating graphs
